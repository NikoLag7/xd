...
if (!empty($_POST['usuario']) && !empty($_POST['contrasena'])){
  ...
  $login = $base_datos->prepare('SELECT id, usuario, contrasena from persona where usuario=:usuario');

  $login->bindparam(':usuario', $_POST['usuario']);
  $login->execute();
  $resultados = $login->fetch(PDO::FETCH_ASSOC);

  $msm='';

  if (!empty($resultados) && password_verify($_POST['contrasena'], $resultados['contrasena'])){

    $_SESSION['persona_id'] = $resultados['id'];
    header('locationn: /php-login');
  } else {

    $msm='el Usuario y Contraseña no coinciden';
  }
}
